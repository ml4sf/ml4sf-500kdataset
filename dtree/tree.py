...
def get_data_train_tst(train_csv, tst_csv, cols, trh, target):
    ...
    # Fit StandardScaler on training dataset
    scaler.fit(X_train)
...
if __name__ == '__main__':
    ...
    if (len(sys.argv) != 4 ):
        print("usage: {} <train>.csv <test>.csv <descriptors>.json".format(sys.argv[0]))
        exit(1)
    ...
    depth = None # if max_depth is set to -1, use maximally deep tree (i.e. pure leaf classes)
    if (d["max_depth"] > 0):
        depth = d["max_depth"]
